    ...
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: [
          "'self'",
          "wss:",
          "https://api.waha.devlike.pro",
          "https://waha-api.vercel.app",
        ],
        frameSrc: ["'self'"],
        objectSrc: ["'none'"],
...
(async () => {
      ...
      modules: [
        {
          ...
          description: "Controle de usuários e perfis",
          active: true,
        },
        {
          id: 2,
        ...
        },
        {
          ...
          description: "Análises estatísticas premium",
          active: true,
        },
        {
          id: 6,
        ...
        {
          id: 7,
          name: "Recursos Enterprise",
          description: "Recursos exclusivos para empresas",
          active: true,
        },
      ],
    ...
    },
    {
      ...
      modules: [
        ...
        },
        {
          ...
          description: "Análises estatísticas e relatórios avançados",
          active: true,
        },
        {
          id: 4,
          name: "Exportação de Relatórios",
          description: "Exportação de relatórios em diversos formatos",
          active: true,
        },
        {
          id: 6,
  ...
  app.put("/api/organizations/:id/status", (req, res) => {
    ...
    const index = global.mockOrganizations.findIndex(
    ...
    try {
      ...
      if (status === "active") {
        ...
        if (
          !global.mockOrganizations[index].planId ||
          global.mockOrganizations[index].planId === 0
        ) {
        ...
        if (
          ...
          !Array.isArray(global.mockOrganizations[index].modules) ||
          !global.mockOrganizations[index].modules.length
        ) {
          global.mockOrganizations[index].modules = [];
  ...
  async function sendConfirmationEmail(
    ...
    console.log(
      `Assunto: Bem-vindo(a) à plataforma! Acesse sua organização ${organizationName}`,
    );
  ...
  app.post("/api/plan-change-requests/approve", async (req, res) => {
    ...
    console.log(
      `Mock: Recebida aprovação para organização ${organizationId} com plano ${planId}`,
    );
    ...
    if (organizationId === 1) {
      try {
        ...
        const orgIndex = global.mockOrganizations.findIndex(
        ...
        if (orgIndex !== -1) {
          ...
          console.log(
            "Atribuindo módulos obrigatórios à organização:",
            org.name,
          );
          for (const module of requiredModules) {
          ...
          console.log(
            "Nova organização criada para demonstração:",
            newOrg.name,
          );
        }
        ...
        return res.status(200).json({
          success: true,
          message:
            "Solicitação de mudança de plano aprovada com sucesso. A organização foi criada e um e-mail de confirmação foi enviado ao administrador.",
          organizationId,
          planId,
    ...
    if (organizationId === 1) {
      ...
      if (orgIndex !== -1) {
        ...
        org.planHistory.push({
          ...
          changeDate: new Date().toISOString(),
          reason:
            "Solicitação de mudança de plano rejeitada pelo administrador",
        });
  ...
  app.get("/api-test/transparencia/organizacao/:orgId", async (req, res) => {
    ...
    try {
      ...
      const safeOrganizacao = {
        ...
        description:
          "Associação dedicada ao acesso e pesquisa com cannabis medicinal.", // Descrição padrão
        socialMediaLinks: null, // Sem redes sociais por padrão
        founding: "2018", // Ano de fundação padrão
        mission:
          "Promoção de pesquisa e acesso a soluções à base de cannabis para tratamentos medicinais",
        vision:
          "Ser referência em pesquisa, desenvolvimento e disseminação de conhecimento sobre o uso medicinal da Cannabis no Brasil",
      ...
      console.error(
        "[API TEST] Erro ao buscar informações da organização:",
        error,
      );
      return res
        .status(500)
  ...
  app.get("/api-test/transparencia/documentos/:orgId", async (req, res) => {
    ...
    try {
      const db = (await import("./db")).db;
      const { documentosTransparencia } = await import(
        "../shared/schema-transparencia"
      );
      ...
      console.log(
        `API Test - Retrieved ${documentos.length} documentos for org ${orgId}`,
      );
      ...
      return res
        .status(500)
        .json({ message: "Erro ao buscar documentos", error: String(error) });
    }
  });
  ...
  app.get("/api-test/transparencia/certificacoes/:orgId", async (req, res) => {
    ...
    try {
      const db = (await import("./db")).db;
      const { certificacoesOrganizacao } = await import(
        "../shared/schema-transparencia"
      );
      const { eq, and } = await import("drizzle-orm");

      const certificacoes = await db
        .select()
        .from(certificacoesOrganizacao)
        .where(eq(certificacoesOrganizacao.organizacaoId, parseInt(orgId, 10)));

      console.log(
        `API Test - Retrieved ${certificacoes.length} certificações for org ${orgId}`,
      );
  ...
  app.get("/api-test/transparencia/membros/:orgId", async (req, res) => {
    ...
    try {
      const db = (await import("./db")).db;
      const { membrosTransparencia } = await import(
        "../shared/schema-transparencia"
      );
      const { eq, and } = await import("drizzle-orm");
      ...
      console.log(
        `API Test - Retrieved ${membros.length} membros for org ${orgId}`,
      );
      ...
      return res
        .status(500)
        .json({ message: "Erro ao buscar membros", error: String(error) });
    }
  });
  ...
  app.get("/api-test/transparencia/financeiro/:orgId", async (req, res) => {
    ...
    try {
      const db = (await import("./db")).db;
      const { relatoriosFinanceirosPublicos } = await import(
        "../shared/schema-transparencia"
      );
      const { eq, and } = await import("drizzle-orm");
      ...
      const relatorios = await db
        ...
        .where(
          eq(relatoriosFinanceirosPublicos.organizacaoId, parseInt(orgId, 10)),
        );
      ...
      const processedRelatorios = relatorios.map((relatorio) => ({
        ...
        receitasPorCategoria: relatorio.receitasPorCategoria
          ? JSON.parse(relatorio.receitasPorCategoria)
          : null,
        despesasPorCategoria: relatorio.despesasPorCategoria
          ? JSON.parse(relatorio.despesasPorCategoria)
          : null,
        receitasMensais: relatorio.receitasMensais
          ? JSON.parse(relatorio.receitasMensais)
          : null,
        despesasMensais: relatorio.despesasMensais
          ? JSON.parse(relatorio.despesasMensais)
          : null,
      }));

      console.log(
        `API Test - Retrieved ${processedRelatorios.length} relatórios financeiros for org ${orgId}`,
      );
  ...
  app.use("/api-json", (req, res, next) => {
    ...
    try {
      ...
      console.log(
        `API-JSON: Autenticado como ${req.session.user.username} (ID: ${req.session.user.id}, Org: ${req.session.user.organizationId})`,
      );
  ...
  function apiJsonIsAssociation(req, res, next) {
    if (
      ...
      return res.status(403).json({
        message:
          "Acesso negado. Apenas administradores de associações podem acessar este recurso.",
        error: "Forbidden",
      });
    }
    next();
  ...
  app.get(
    "/api-json/carteirinha/membership-cards",
    apiJsonAuthenticate,
    apiJsonIsAssociation,
    async (req, res) => {
      try {
        // Forçar Content-Type para JSON
        res.setHeader("Content-Type", "application/json");
        ...
        console.log(
          `API-JSON: Rota direta carteirinhas, usuário:`,
        ...
        try {
          ...
          console.error(
            "API-JSON-Carteirinha: Erro ao consultar banco de dados:",
            dbError,
          );
          return res.status(500).json({
            error: true,
  ...
  app.get(
    "/api-json/carteirinha/membership-cards/settings/current",
    apiJsonAuthenticate,
    apiJsonIsAssociation,
    async (req, res) => {
      try {
        ...
        res.setHeader("Pragma", "no-cache");
        console.log(
          `API-JSON: Rota direta para configurações, usuário:`,
          req.session.user?.username,
        );
        ...
        try {
          ...
          const [settings] = await db
            ...
            .where(
              eq(socialMembershipCardSettings.organizationId, organizationId),
            );
          ...
          console.error(
            "API-JSON-Configurações: Erro ao consultar banco de dados:",
            dbError,
          );
  ...
  console.log(
    "Rotas do Portal de Laboratório e Fornecedores registradas com sucesso",
  );

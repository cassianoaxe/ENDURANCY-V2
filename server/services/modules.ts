...
async function createModuleWithPlans(moduleData: typeof modulesList[0]) {
  try {
    ...
    if (existingModules.length > 0) {
      ...
      console.log(`Módulo ${moduleData.name} já existe, não é necessário atualizar.`);
    ...
    // Verificar apenas se os planos principais existem
    ...
    console.error(`Erro ao verificar/criar módulo ${moduleData.name}:`, error);
  }
}

/**
 * Inicializa os módulos e planos padrão no banco de dados
 * Esta função é executada na inicialização do servidor
 */
export async function initializeModules() {
  console.log("Verificando módulos e planos...");
  
  try {
    // Verificar se existem módulos no banco de dados
    const existingModules = await db.select().from(modules);
    
    if (existingModules.length > 0) {
      console.log(`${existingModules.length} módulos já existem no banco de dados, pulando inicialização.`);
      return;
    }
    
    console.log("Nenhum módulo encontrado. Iniciando seed de módulos e planos...");
    
    // Criar módulos em paralelo
    await Promise.all(modulesList.map(moduleData => createModuleWithPlans(moduleData)));
    
    console.log("Inicialização de módulos e planos concluída!");
  } catch (error) {
    console.error("Erro durante a inicialização de módulos:", error);
  }
}
...
export class WahaWhatsAppAPI {
   ...
   */
  async checkSessionStatus(): Promise<string> {
    try {
   ...
   * @returns {Promise<object>} Resposta da API
   */
  async sendFileMessage(to: string, fileUrl: string, caption: string = '', type: string = 'image'): Promise<any> {
    try {
      if (!to.includes('@')) {
        to = to.replace(/\D/g, '') + '@c.us';
      }
 ...
 */
export function setupWahaWebhook(app: any, messageHandler: (message: any) => void): void {
  app.post("/api/webhooks/whatsapp", express.json(), (req: any, res: any) => {
    const data = req.body;
 ...
 */
export function exampleMessageProcessor(message: any): void {

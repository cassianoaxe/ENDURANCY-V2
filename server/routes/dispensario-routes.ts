...
const dispensarioRouter = Router();

// Middleware para verificar se o usuário tem acesso ao módulo de dispensário
const checkDispensarioModuleAccess = async (req: Request, res: Response, next: Function) => {
  try {
    if (!req.session.user) {
    ...
    // Verificar se o módulo de dispensário está ativado para esta organização
    const moduleCheck = await db.select({
      ...
      .where(
        and(
          ...
          eq(modules.slug, 'dispensario')
        )
      )
      .limit(1);

    if (moduleCheck.length === 0) {
      return res.status(403).json({ message: 'Módulo de dispensário não está disponível para esta organização' });
    }
    
    // Verificar se o módulo está ativo
    if (!moduleCheck[0].active) {
      return res.status(403).json({ message: 'Módulo de dispensário está desativado para esta organização' });
    }

    next();
  } catch (error) {
    console.error('Erro ao verificar acesso ao módulo de dispensário:', error);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
};

// Obter status do módulo de dispensário
dispensarioRouter.get('/status', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    ...
    if (moduleData.length === 0) {
      return res.status(404).json({ message: 'Módulo de dispensário não encontrado' });
    }
    ...
    console.error('Erro ao obter status do módulo de dispensário:', error);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
});

// Alternar status do módulo (ativar/desativar)
dispensarioRouter.put('/toggle', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    ...
    console.error('Erro ao alternar status do módulo de dispensário:', error);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
});

// Atualizar configurações do módulo
dispensarioRouter.put('/settings', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    ...
    console.error('Erro ao atualizar configurações do módulo de dispensário:', error);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
});

// Obter métricas do módulo de dispensário
dispensarioRouter.get('/metrics', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    ...
    console.error('Erro ao obter métricas do módulo de dispensário:', error);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
});

// Obter lista de farmacêuticos RT
dispensarioRouter.get('/farmacistas', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
...
dispensarioRouter.post('/farmacistas', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
...
dispensarioRouter.get('/farmacistas/:id', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
...
dispensarioRouter.put('/farmacistas/:id', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    const farmaceuticoId = parseInt(req.params.id);
...
dispensarioRouter.patch('/farmacistas/:id/status', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    const farmaceuticoId = parseInt(req.params.id);
...
dispensarioRouter.delete('/farmacistas/:id', authenticate, checkDispensarioModuleAccess, async (req, res) => {
  try {
    const { organizationId } = req.session.user || {};
    const farmaceuticoId = parseInt(req.params.id);

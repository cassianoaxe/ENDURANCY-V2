...
// Criar um Router para as rotas de treinamento HPLC
const hplcTrainingRouter = Router();

// GET all trainings
hplcTrainingRouter.get('/trainings', async (req: Request, res: Response) => {
  try {
...
// GET training by ID
hplcTrainingRouter.get('/trainings/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const laboratoryId = req.session.user?.organizationId;
...
hplcTrainingRouter.post('/trainings', async (req: Request, res: Response) => {
  try {
    const {
      trainingTitle,
      trainingType,
...
hplcTrainingRouter.put('/trainings/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const {
      trainingTitle,
...
hplcTrainingRouter.delete('/trainings/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const laboratoryId = req.session.user?.organizationId;
...
hplcTrainingRouter.get('/training-users', async (req: Request, res: Response) => {
  try {
    const laboratoryId = req.session.user?.organizationId;
...
hplcTrainingRouter.get('/training-procedures', async (req: Request, res: Response) => {
  try {
    const laboratoryId = req.session.user?.organizationId;
...
hplcTrainingRouter.get('/training-equipments', async (req: Request, res: Response) => {
  try {
    const laboratoryId = req.session.user?.organizationId;
...
export async function registerHplcTrainingRoutes(app: Express) {
  // Usando o prefixo API explicitamente para evitar conflito com o middleware do Vite
  app.use('/api/laboratory/hplc', authenticate, checkHplcAccess, hplcTrainingRouter);
  console.log('Rotas HPLC de treinamento registradas com sucesso');
}
...
export default function SupplierProducts() {
  ...
  // Dados simulados para a página de produtos
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (filter !== "all") {
      if (filter === "low_stock" && product.status !== "low_stock" && product.status !== "out_of_stock") {
        return false;
      } else if (filter !== "low_stock" && product.status !== filter) {
        return false;
      }
    }
  ...
  // Ordenação de produtos (por vendas e depois por nome)
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    // Primeiro por vendas (decrescente)
    if (a.sales !== b.sales) {
  ...
  const saveProductChanges = () => {
    toast({
      ...
      description: `As alterações no produto "${selectedProduct?.name}" foram salvas com sucesso.`,
    });
    closeProductDetails();
  };
  ...
  const changeProductStatus = (productId: number, status: string) => {
    toast({
      ...
      description: `O status do produto foi alterado para "${getStatusText(status)}".`,
    });
  };
        ...
        <Tabs defaultValue="all" className="mb-6">
          <TabsList>
            <TabsTrigger value="all" onClick={() => setFilter("all")}>
              Todos
              <Badge className="ml-2 bg-gray-200 text-gray-700">{products.length}</Badge>
            </TabsTrigger>
            <TabsTrigger value="active" onClick={() => setFilter("active")}>
              Ativos
              <Badge className="ml-2 bg-green-100 text-green-700">{products.filter(p => p.status === "active").length}</Badge>
            </TabsTrigger>
            ...
            <TabsTrigger value="draft" onClick={() => setFilter("draft")}>
              Rascunhos
              <Badge className="ml-2 bg-blue-100 text-blue-700">{products.filter(p => p.status === "draft").length}</Badge>
            </TabsTrigger>
          </TabsList>
        </Tabs>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-gray-600">
                    Estoque: <span className={product.stock === 0 ? "text-red-600 font-bold" : product.stock < 10 ? "text-yellow-600 font-bold" : ""}>
                      {product.stock}
                    </span>
                  </div>
                </div>
